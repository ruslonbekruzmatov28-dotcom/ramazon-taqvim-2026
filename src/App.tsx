...
const App: React.FC = () => {
  ...
  const requestNotificationPermission = async () => {
    ...
    try {
      ...
      if (permission === 'granted') {
        ...
        new Notification("Muvaffaqiyatli!", { body: "Endi eslatmalar sizga yuboriladi." });
      } else {
        alert("Bildirishnomalar uchun ruxsat berilmadi. Iltimos, brauzer sozlamalaridan ruxsat bering.");
      }
    } catch (err) {
  ...
  const sendTelegramNotification = async (text: string) => {
    if (!notifications.telegramEnabled || !notifications.telegramChatId || !notifications.telegramBotToken) return;
  ...
  useEffect(() => {
    if (!notifications.enabled && !notifications.telegramEnabled) return;

    const checkNotifications = () => {
      const now = new Date();
      
      const checkTime = (targetTimeStr: string, label: string, body: string) => {
        ...
        if (now.getHours() === reminderTime.getHours() && now.getMinutes() === reminderTime.getMinutes() && now.getSeconds() === 0) {
          ...
          if (notifications.telegramEnabled) {
            sendTelegramNotification(`🔔 *${label}*\n\n${body}`);
          }
        }
      };

      if (notifications.saharReminder) {
        checkTime(todayData.fajr, "Saharlik yaqinlashmoqda!", `Saharlik vaqtiga ${notifications.reminderMinutes} daqiqa qoldi. Bugungi niyatni unutmang!`);
      }

      if (notifications.iftorReminder) {
        checkTime(todayData.maghrib, "Iftorlik yaqinlashmoqda!", `Iftorlik vaqtiga ${notifications.reminderMinutes} daqiqa qoldi. Alloh qabul qilsin!`);
      }
    };
            ...
            <div className="space-y-4 pb-24">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-black text-gray-800">Brauzer bildirishnomasi</h3>
                    <p className="text-xs text-gray-400">Ilova ochiq turganda eslatadi</p>
                  </div>
                  <button 
                    onClick={() => {
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                  <div>
                    <h3 className="font-black text-gray-800">Telegram orqali eslatma</h3>
                    <p className="text-xs text-gray-400">Eng ishonchli usul (Bot orqali)</p>
                  </div>
                  <button 
                    onClick={() => setNotifications(prev => ({ ...prev, telegramEnabled: !prev.telegramEnabled }))}
                    className={`w-14 h-8 rounded-full transition-all relative ${notifications.telegramEnabled ? 'bg-blue-500' : 'bg-gray-200'}`}
                  >
                    <motion.div 
                      animate={{ x: notifications.telegramEnabled ? 24 : 4 }}
                      className="absolute top-1 w-6 h-6 bg-white rounded-full shadow-sm"
                    />
                  </button>
                </div>
                ...
                {notifications.telegramEnabled && (
                  ...
                  >
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Bot Token</label>
                      <input 
                        type="password" 
                        placeholder="BotFather bergan token"
                        value={notifications.telegramBotToken}
                        onChange={(e) => setNotifications(prev => ({ ...prev, telegramBotToken: e.target.value }))}
                        className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-400 outline-none"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Telegram Chat ID</label>
                      <input 
                        type="text" 
                        placeholder="Sizning Chat ID"
                      ...
                      <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 space-y-2">
                        ...
                        <ol className="text-[8px] text-blue-600 space-y-1 list-decimal pl-4">
                          <li>Telegramda <a href="https://t.me/BotFather" target="_blank" className="underline font-bold">@BotFather</a> orqali bot oching va <b>Token</b> oling.</li>
                          <li>O'sha botga kirib <b>/start</b> bosing.</li>
                          <li><a href="https://t.me/userinfobot" target="_blank" className="underline font-bold">@userinfobot</a> orqali o'z <b>Chat ID</b> raqamingizni oling.</li>
                        </ol>
                      </div>
                    </div>
                  </motion.div>
                ...
                {(notifications.enabled || notifications.telegramEnabled) && (
                  <motion.div 
                    initial={{ opacity: 0, height: 0 }}
                    animate={{ opacity: 1, height: 'auto' }}
                  ...
                  >
                    ...
                    <button 
                      onClick={() => {
                        if (notifications.enabled) new Notification("Test", { body: "Eslatmalar ishlamoqda!" });
                        if (notifications.telegramEnabled) sendTelegramNotification("✅ *Test xabari*\n\nTelegram orqali eslatmalar muvaffaqiyatli sozlandi!");
                      }}
